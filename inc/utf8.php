...
if(!function_exists('utf8_deaccent')){
    ...
    function utf8_deaccent($string,$case=0){
        if($case <= 0){
            global $UTF8_LOWER_ACCENTS,$UTF8_LOWER_DEACCENTS;
            $string = str_replace($UTF8_LOWER_ACCENTS,$UTF8_LOWER_DEACCENTS,$string);
        }
        if($case >= 0){
            global $UTF8_UPPER_ACCENTS,$UTF8_UPPER_DEACCENTS;
            $string = str_replace($UTF8_UPPER_ACCENTS,$UTF8_UPPER_DEACCENTS,$string);
        }
        return $string;
    }
}
...
if(!function_exists('utf8_split')){
    /**
     * split a utf8 string into an array
     *
     * @param   $str    string  utf8 character string
     * @param   $split  int     maximum length of each chunk
     * @return  array           array of chunks with a maximum of $split utf8 characters in each
     * @author Tom N Harris <tnharris@whoopdedo.org>
     */
    function utf8_split($str, $split=1) {
...
if(empty($UTF8_LOWER_ACCENTS)) $UTF8_LOWER_ACCENTS = array(
    'ỳ', 'ẅ', 'ẃ', 'ẁ', 'ṫ', 'ṡ', 'ṗ', 'ṁ', 'ḟ', 'ḋ', 'ḃ', 'ț', 'ș', 'ư', 'ơ',
    'ƒ', 'ž', 'ż', 'ź', 'ŷ', 'ŵ', 'ų', 'ű', 'ů', 'ŭ', 'ū', 'ũ', 'ŧ', 'ť', 'ţ',
    'š', 'ş', 'ŝ', 'ś', 'ř', 'ŗ', 'ŕ', 'ő', 'ō', 'ň', 'ņ', 'ń', 'ł', 'ľ', 'ļ',
    'ĺ', 'ķ', 'ĵ', 'į', 'ī', 'ĩ', 'ħ', 'ĥ', 'ģ', 'ġ', 'ğ', 'ĝ', 'ě', 'ę', 'ė',
    'ĕ', 'ē', 'đ', 'ď', 'č', 'ċ', 'ĉ', 'ć', 'ą', 'ă', 'ā', 'ÿ', 'þ', 'ý', 'ü',
    'û', 'ú', 'ù', 'ø', 'ö', 'õ', 'ô', 'ó', 'ò', 'ñ', 'ð', 'ï', 'î', 'í', 'ì',
    'ë', 'ê', 'é', 'è', 'ç', 'æ', 'å', 'ä', 'ã', 'â', 'á', 'à', 'ß', 'µ',
);
global $UTF8_LOWER_DEACCENTS;
if(empty($UTF8_LOWER_DEACCENTS)) $UTF8_LOWER_DEACCENTS = array(
    'y', 'w', 'w', 'w', 't', 's', 'p', 'm', 'f', 'd', 'b', 't', 's', 'u',
    'o', 'f', 'z', 'z', 'z', 'y', 'w', 'u', 'u', 'u', 'u', 'u', 'u', 't',
    't', 't', 's', 's', 's', 's', 'r', 'r', 'r', 'o', 'o', 'n', 'n', 'n',
    'l', 'l', 'l', 'l', 'k', 'j', 'i', 'i', 'i', 'h', 'h', 'g', 'g', 'g',
    'g', 'e', 'e', 'e', 'e', 'e', 'd', 'd', 'c', 'c', 'c', 'c', 'a', 'a',
    'a', 'y', 'th', 'y', 'ue', 'u', 'u', 'u', 'o', 'oe', 'o', 'o', 'o', 'o',
    'n', 'dh', 'i', 'i', 'i', 'i', 'e', 'e', 'e', 'e', 'c', 'ae', 'a', 'ae',
    'a', 'a', 'a', 'a', 'ss', 'u',
);
...
if(empty($UTF8_UPPER_ACCENTS)) $UTF8_UPPER_ACCENTS = array(
    'Ỳ', 'Ẅ', 'Ẃ', 'Ẁ', 'Ṫ', 'Ṡ', 'Ṗ', 'Ṁ', 'Ḟ', 'Ḋ', 'Ḃ', 'Ț', 'Ș', 'Ư', 'Ơ',
    'Ƒ', 'Ž', 'Ż', 'Ź', 'Ÿ', 'Ŷ', 'Ŵ', 'Ų', 'Ű', 'Ů', 'Ŭ', 'Ū', 'Ũ', 'Ŧ', 'Ť',
    'Ţ', 'Š', 'Ş', 'Ŝ', 'Ś', 'Ř', 'Ŗ', 'Ŕ', 'Ő', 'Ō', 'Ň', 'Ņ', 'Ń', 'Ł', 'Ľ',
    'Ļ', 'Ĺ', 'Ķ', 'Ĵ', 'Į', 'Ī', 'Ĩ', 'Ħ', 'Ĥ', 'Ģ', 'Ġ', 'Ğ', 'Ĝ', 'Ě', 'Ę',
    'Ė', 'Ĕ', 'Ē', 'Đ', 'Ď', 'Č', 'Ċ', 'Ĉ', 'Ć', 'Ą', 'Ă', 'Ā', 'Þ', 'Ý', 'Ü',
    'Û', 'Ú', 'Ù', 'Ø', 'Ö', 'Õ', 'Ô', 'Ó', 'Ò', 'Ñ', 'Ð', 'Ï', 'Î', 'Í', 'Ì',
    'Ë', 'Ê', 'É', 'È', 'Ç', 'Æ', 'Å', 'Ä', 'Ã', 'Â', 'Á', 'À', 
);
global $UTF8_UPPER_DEACCENTS;
if(empty($UTF8_UPPER_DEACCENTS)) $UTF8_UPPER_DEACCENTS = array(
    'Y', 'W', 'W', 'W', 'T', 'S', 'P', 'M', 'F', 'D', 'B', 'T', 'S', 'U',
    'O', 'F', 'Z', 'Z', 'Z', 'Y', 'Y', 'W', 'U', 'U', 'U', 'U', 'U', 'U',
    'T', 'T', 'T', 'S', 'S', 'S', 'S', 'R', 'R', 'R', 'O', 'O', 'N', 'N',
    'N', 'L', 'L', 'L', 'L', 'K', 'J', 'I', 'I', 'I', 'H', 'H', 'G', 'G',
    'G', 'G', 'E', 'E', 'E', 'E', 'E', 'D', 'D', 'C', 'C', 'C', 'C', 'A',
    'A', 'A', 'Th', 'Y', 'Ue', 'U', 'U', 'U', 'O', 'Oe', 'O', 'O', 'O', 'O',
    'N', 'Dh', 'I', 'I', 'I', 'I', 'E', 'E', 'E', 'E', 'C', 'Ae', 'A', 'Ae',
    'A', 'A', 'A', 'A', 
);
 ...
 */
/* The character codes are not used anywhere
global $UTF8_SPECIAL_CHARS;
...
if(empty($UTF8_SPECIAL_CHARS2)) $UTF8_SPECIAL_CHARS2 = array(
    ...
    '＜','＝','＞','？','＠','［','＼','］','＾','｀','｛','｜','｝','～','｟','｠','｡',
    '｢','｣','､','･','￠','￡','￢','￣','￤','￥','￦','￨','￩','￪','￫','￬','￭','￮',
    '𝛼','𝛽','𝛾','𝛿','𝜀','𝜁','𝜂','𝜃','𝜄','𝜅','𝜆','𝜇','𝜈','𝜉','𝜊','𝜋',
