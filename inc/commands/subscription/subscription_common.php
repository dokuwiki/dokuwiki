...
abstract class Doku_Action_Subscription_Common extends Doku_Action
{
    ...
    function handle(){
        ...
        if($ok) {
            msg(sprintf($lang["subscr_{$act}_success"], hsc($INFO['userinfo']['name']),
                        prettyprint_id($target)), 1);
        }else{
            msg(sprintf($lang["subscr_{$act}_error"],
...
/**
 * The commons between the subscribe and unsubscribe renderers
 * 
 * @author Junling Ma <junlingm@gmail.com>
 */
abstract class Doku_Action_Renderer_Subscription_Common extends Doku_Action_Renderer
{
    /**
