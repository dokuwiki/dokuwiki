...
class ApiCore
{
    ...
    public function search($query)
    {
        ...
        foreach ($data as $id => $score) {
            ...
            $pages[] = ['id' => $id, 'score' => (int) $score, 'rev' => filemtime($file), 'mtime' => filemtime($file), 'size' => filesize($file), 'snippet' => $snippet, 'title' => useHeading('navigation') ? p_get_first_heading($id) : $id];
        }
        return $pages;
    }
    ...
    public function listAttachments($ns, $options = [])
    {
        ...
        if (auth_quickaclcheck($ns . ':*') >= AUTH_READ) {
            ...
            $len = is_countable($data) ? count($data) : 0;
            if (!$len) return [];
    ...
    public function pageInfo($id, $rev = '')
    {
        ...
        $data = ['name' => $id, 'lastModified' => $this->api->toDate($rev), 'author' => is_array($info) ? ($info['user'] ?: $info['ip']) : null, 'version' => $rev];
    ...
    public function setLocks($set)
    {
        ...
        return ['locked' => $locked, 'lockfail' => $lockfail, 'unlocked' => $unlocked, 'unlockfail' => $unlockfail];
    }
    ...
    public function login($user, $pass)
    {
        ...
        if ($ok === null){
            $evdata = ['user' => $user, 'password' => $pass, 'sticky' => false, 'silent' => true];
