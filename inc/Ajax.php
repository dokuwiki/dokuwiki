...
class Ajax {
    ...
    protected function callMediaupload() {
        ...
        if($res) {
            $result = ['success' => true, 'link' => media_managerURL(['ns' => $ns, 'image' => $NS . ':' . $id], '&'), 'id' => $NS . ':' . $id, 'ns' => $NS];
        } else {
            $error = '';
            if(isset($MSG)) {
                foreach($MSG as $msg) {
    ...
    protected function callLinkwiz() {
        ...
        if($q !== '' && $ns === '') {
            ...
            $opts = ['depth' => 1, 'listfiles' => true, 'listdirs' => true, 'pagesonly' => true, 'firsthead' => true, 'sneakyacl' => $conf['sneaky_index']];
