...
{
    if (media_isexternal($id)) {
        return true;
    }
    $id = cleanID($id);
    if (auth_aclcheck(getNS($id) . ':*', '', []) >= AUTH_READ) {
        return true;
    }
    return false;
}
...
function media_nstree_item($item)
{
    ...
    if ($INPUT->str('do') != 'media') {
        ...
        $ret .= '<a href="' . media_managerURL(['ns' => idfilter($item['id'], false), 'tab_files' => 'files'])
            . '" class="idx_dir">';
    }
    $ret .= $item['label'];
    $ret .= '</a>';
