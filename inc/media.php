...
{
    return (bool) preg_match('#^(?:https?|ftp)://#i', $id);
}
...
function media_ispublic($id){
    ...
    $id = cleanID($id);
    return auth_aclcheck(getNS($id).':*', '', []) >= AUTH_READ;
}
...
function media_metaform($id, $auth) {
    ...
    foreach ($fields as $key => $field) {
        ...
        // prepare attributes
        $p = ['class' => 'edit', 'id'    => 'meta__'.$key, 'name'  => 'meta['.$field[0].']'];
...
function media_upload($ns,$auth,$file=false){
    ...
    $res = media_save(['name' => $file['tmp_name'], 'mime' => $imime, 'ext'  => $iext], $ns.':'.$id,
...
function media_tabs_files($selected_tab = ''){
    global $lang;
    $tabs = [];
    foreach(['files'  => 'mediaselect', 'upload' => 'media_uploadtab', 'search' => 'media_searchtab'] as $tab => $caption) {
        $tabs[$tab] = ['href'    => media_managerURL(['tab_files' => $tab], '&'), 'caption' => $lang[$caption]];
    }
...
function media_tabs_details($image, $selected_tab = '') {
    ...
    $tabs = [];
    $tabs['view'] = ['href'    => media_managerURL(['tab_details' => 'view'], '&'), 'caption' => $lang['media_viewtab']];

    [, $mime] = mimetype($image);
    if ($mime == 'image/jpeg' && file_exists(mediaFN($image))) {
        $tabs['edit'] = ['href'    => media_managerURL(['tab_details' => 'edit'], '&'), 'caption' => $lang['media_edittab']];
    }
    if ($conf['mediarevisions']) {
        $tabs['history'] = ['href'    => media_managerURL(['tab_details' => 'history'], '&'), 'caption' => $lang['media_historytab']];
    }
...
function media_tab_files_options() {
    ...
    $form->addHTML('<ul>'.NL);
    foreach (['list' => ['listType', ['thumbs', 'rows']], 'sort' => ['sortBy', ['name', 'date']]]
            as $group => $content) {
...
function media_searchlist($query,$ns,$auth=null,$fullscreen=false,$sort='natural'){
    ...
    if(!(is_countable($evdata['data']) ? count($evdata['data']) : 0)){
        echo '<div class="nothing">'.$lang['nothingfound'].'</div>'.NL;
    }else {
        if ($fullscreen) {
            echo '<ul class="' . _media_get_list_type() . '">';
...
function media_nstree($ns){
    ...
    foreach ($ns_parts as $level => $part) {
        ...
        while ($data[$pos]['id'] != $tmp_ns) {
            if (
                $pos >= (is_countable($data) ? count($data) : 0) ||
...
function media_supportedav($mime, $type=NULL){
    $supportedAudio = ['ogg' => 'audio/ogg', 'mp3' => 'audio/mpeg', 'wav' => 'audio/wav'];
    $supportedVideo = ['webm' => 'video/webm', 'ogv' => 'video/ogg', 'mp4' => 'video/mp4'];
    if ($type == 'audio') {
...
function media_trackfiles($src){
    $kinds=['sub' => 'subtitles', 'cap' => 'captions', 'des' => 'descriptions', 'cha' => 'chapters', 'met' => 'metadata'];
