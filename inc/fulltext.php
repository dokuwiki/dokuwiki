...
class FulltextSearch
{
     ...
     */
    public static function queryParser($Indexer, $query)
    {
        /**
        ...
        foreach ($terms as $term) {
            ...
            if (preg_match('/^(-?)"(.+)"$/u', $term, $matches)) {
                ...
                $not = $matches[1] ? 'NOT' : '';
                $parsed = $not . static::termParser($Indexer, $matches[2], false, true);
            } else {
                ...
                foreach ($tokens as $token) {
                    if ($token === '(') {
                        ...
                        // word-exclude
                        $parsed .= 'NOT('.static::termParser($Indexer, $matches[1]).')';
                    } else {
                        // word-include
                        $parsed .= static::termParser($Indexer, $token);
                    }
                }
            }
            $parsed_query .= $parsed;
     ...
     * @return string
     */
    public static function queryUnparser_simple(
