...
function auth_setup() {
    ...
    if ($ok === null) {
        ...
        $evdata = ['user'     => $INPUT->str('u'), 'password' => $INPUT->str('p'), 'sticky'   => $INPUT->bool('r'), 'silent'   => $INPUT->bool('http_credentials')];
...
function auth_aclcheck($id, $user, $groups) {
    $data = ['id'     => $id ?? '', 'user'   => $user, 'groups' => $groups];
...
function auth_sendPassword($user, $password) {
    ...
    $trep = ['FULLNAME' => $userinfo['name'], 'LOGIN'    => $user, 'PASSWORD' => $password];
