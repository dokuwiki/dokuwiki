...
function clientismobile() {
    ...
    return (bool) preg_match("/{$uamatches}/i", $INPUT->server->str('HTTP_USER_AGENT'));
}
...
{
    return (bool) preg_match('/^[a-zA-Z0-9\.]+>/u',$link);
}
...
function pageTemplate($id) {
    ...
    $data = [
        'id'        => $id,
        // the id of the page to be created
        'tpl'       => '',
        // the text used as template
        'tplfile'   => '',
        // the file above text was/should be loaded from
        'doreplace' => true,
    ];
...
function parsePageTemplate(&$data) {
    ...
    $tpl = str_replace(
        ['@ID@', '@NS@', '@CURNS@', '@!CURNS@', '@!!CURNS@', '@!CURNS!@', '@FILE@', '@!FILE@', '@!FILE!@', '@PAGE@', '@!PAGE@', '@!!PAGE@', '@!PAGE!@', '@USER@', '@NAME@', '@MAIL@', '@DATE@'],
        [$id, getNS($id), curNS($id), PhpString::ucfirst(curNS($id)), PhpString::ucwords(curNS($id)), PhpString::strtoupper(curNS($id)), $file, PhpString::ucfirst($file), PhpString::strtoupper($file), $page, PhpString::ucfirst($page), PhpString::ucwords($page), PhpString::strtoupper($page), $INPUT->server->str('REMOTE_USER'), $USERINFO ? $USERINFO['name'] : '', $USERINFO ? $USERINFO['mail'] : '', $conf['dformat']], $tpl
    );
...
function userlink($username = null, $textonly = false) {
    ...
    $data = [
        ...
        'userlink' => '',
        // formatted user name as will be returned
        'textonly' => $textonly,
    ];
    if($username === null) {
