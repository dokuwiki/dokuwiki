...
class helper_plugin_extension_extension extends DokuWiki_Plugin
{
    ...
    private $localInfo;
    private $remoteInfo;
    private ?array $managerData = null;
    ...
    public function isBundled()
    {
        ...
        return in_array(
            $this->id,
            ['authad', 'authldap', 'authpdo', 'authplain', 'acl', 'config', 'extension', 'info', 'popularity', 'revert', 'safefnrecode', 'styling', 'testing', 'usermanager', 'logviewer', 'template:dokuwiki']
        );
    }
    ...
    public function installArchive($file, $overwrite = false, $base = '')
    {
        ...
        if (is_countable($result['new']) ? count($result['new']) : 0) {
            $install = $result['new'];
        } else {
            $install = $result['old'];
        }

        if (!(is_countable($install) ? count($install) : 0)) {
            throw new Exception($this->getLang('error_findfolder'));
        }
        ...
        foreach ($install as $item) {
            ...
            } elseif ($base && (is_countable($install) ? count($install) : 0) == 1) {
                $item['base'] = $base;
            } else {
            ...
            if ($this->dircopy($item['tmp'], $target)) {
                ...
                $installed_extensions[$id] = ['base' => $item['base'], 'type' => $item['type'], 'action' => $action];
            } else {
