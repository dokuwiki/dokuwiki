...
class auth_plugin_authpdo extends DokuWiki_Auth_Plugin {
    ...
    public function __construct() {
        ...
        if($this->getConf('dbType') == 'mysql'){
            $this->dbName = 'MySQL';
        }
        else if($this->getConf('dbType') == 'pgsql'){
            $this->dbName = 'PgSQL';
        }

        if (!class_exists('PDO') || !in_array($this->getConf('dbType'), PDO::getAvailableDrivers())) {
            $this->_debug($this->dbName." err: PDO extension not found.", -1, __LINE__, __FILE__);
            $this->success = false;
            return;
        }
        ...
        if (!$this->getConf('server') || !$this->getConf('user') || !$this->getConf('database')) {
            $this->_debug($this->dbName." err: insufficient configuration.", -1, __LINE__, __FILE__);
        ...
        if ($this->getConf('debug') >= 2) {
            ...
            $this->_debug("auth" . $this->getConf('dbType') . " cando: " . $candoDebug, 0, __LINE__, __FILE__);
        }
    }
        ...
        if ($this->dbcon && is_array($grps)) {
            ...
            if ($uid) {
                ...
                if ($gid !== false) {
                     ...
                     */
                    $this->_delUser($user);
                    $this->_debug($this->dbName." err: Adding user '$user' to group '$group' failed.", -1, __LINE__, __FILE__);
                }
            }
        }
        return false;
    ...
    protected function _openDB() {
        if (!$this->dbcon) {
            $dsn = $this->getConf('dbType') . ':dbname=' . $this->getConf('database') . ';host=' . $this->getConf('server') . '';
            try {
                ...
                $this->_debug(
                        $this->dbName." err: Connection to {$this->getConf('user')}@{$this->getConf('server')}, database {$this->getConf('database')} not possible.", -1, __LINE__, __FILE__
                );
            }
    ...
    protected function _queryDB($query) {
        if ($this->getConf('debug') >= 2) {
            msg($this->dbName.' query: ' . hsc($query), 0, __LINE__, __FILE__);
        }
        ...
        if ($this->dbcon) {
            ...
            $this->_debug($this->dbName.' err: ' . $this->dbcon->errorInfo(), -1, __LINE__, __FILE__);
        }
        return false;
    }
    ...
    protected function _modifyDB($query) {
        ...
        if ($this->dbcon) {
            ...
            $this->_debug($this->dbName.' err: ' . pg_last_error($this->dbcon), -1, __LINE__, __FILE__);
        }
        return false;
    }
