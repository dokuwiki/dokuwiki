...
class admin_plugin_usermanager extends DokuWiki_Admin_Plugin
{
        ...
        if ($this->users_total) {
            ...
            foreach ($user_list as $user => $userinfo) {
                ...
                ptln("      <td class=\"centeralign\"><input type=\"checkbox\" name=\"delete[".hsc($user).
                     "]\" ".$delete_disable." /></td>");
                if ($editable) {
                    ptln("    <td><a href=\"".wl($ID, ['fn[edit]['.$user.']' => 1, 'do' => 'admin', 'page' => 'usermanager', 'sectok' => getSecurityToken()]).
    ...
    protected function exportCSV()
    {
        ...
        $column_headings = [$this->lang["user_id"], $this->lang["user_name"], $this->lang["user_mail"], $this->lang["user_groups"]];
