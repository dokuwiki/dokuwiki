...
var dw_mediamanager = {
    ...
    update_content: function ($content, params, update_list) {
        ...
        jQuery.post(
            ...
            function (data) {
                ...
                dw_mediamanager.showonlyownmedia();
                dw_mediamanager.showonlyrecentlyuploaded();
    ...
    attachoptions: function(){
        ...
        opts.push(['onlyownmedia', 'showonlyownmedia']);
        opts.push(['onlyrecentlyuploaded', 'showonlyrecentlyuploaded']);
    ...
    toggleOption: function (variable) {
        ...
        if (variable === 'hide') {
            dw_mediamanager.updatehide();
        } else if (variable === 'onlyownmedia') {
            dw_mediamanager.showonlyownmedia();
        } else if (variable === 'onlyrecentlyuploaded') {
            dw_mediamanager.showonlyrecentlyuploaded();
        }
    },
    ...
    showonlyownmedia: function () {
        jQuery('#media__onlyrecentlyuploaded').prop('checked', false);
        jQuery('.odd,.even').each(function(_, element) {
            $e = jQuery(element);
            $user = $e.find('.uploader').text();
            if(dw_mediamanager.onlyownmedia && $user != JSINFO.user) {
                $e.hide();
            } else {
                $e.show();
            }
    ...
    showonlyrecentlyuploaded: function() {
        jQuery('#media__onlyownmedia').prop('checked', false);
        jQuery('.odd,.even').each(function(_, element) {
            $e = jQuery(element);
            //date = $e.find('i[data-timestamp]').attr('data-timestamp');
            date = $e.find('i').data('timestamp');
            date = new Date(Math.round(date * 1000));
            now = new Date(jQuery.now());
            if(dw_mediamanager.onlyrecentlyuploaded && (now - date) > 3600 * 1000) {
                // skip all files which are older than one hour
                $e.hide();
            } else {
                $e.show();
            }
