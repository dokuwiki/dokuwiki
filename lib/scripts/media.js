...
var dw_mediamanager = {
    ...
    insert: function (id) {
        ...
        edid = edid ? edid[1].replace(/[^\w-]/g, '') : 'wiki__text';
        cb = String.prototype.match.call(document.location, /[?&]onselect=([^&]+)/);
        cb = cb ? cb[1].replace(/[^\w]/g, '') : 'dw_mediamanager_item_select';

        // The callback name comes from a URL parameter and is called on the
        // opener. Only accept functions defined in page script, never a
        // browser/JS built-in (eval, Function, setTimeout, open, ...) which all
        // report "[native code]" when stringified.
        var fn = opener[cb];
        if (typeof fn !== 'function' || /\[native code\]/.test(Function.prototype.toString.call(fn))) {
            cb = 'dw_mediamanager_item_select';
        }
