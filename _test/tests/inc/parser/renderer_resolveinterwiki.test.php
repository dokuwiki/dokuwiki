...
/**
 * Tests for Wiki_Renderer::_resolveInterWiki()
 */
class Test_resolveInterwiki extends EasyWikiTest {

    function testDefaults() {
        $Renderer = new Wiki_Renderer();
        ...
        $tests = array(
            ...
            array('wiki', 'foo [\\]^`{|}~@+#%?/#txt', 'https://www.dokuwiki.org/foo%20%5B%5C%5D%5E%60%7B%7C%7D~%40%2B%23%25%3F%2F#txt'),
            ...
            //easywiki id's
            array('onlytext', 'foo [\\]^`{|}~@+#%/#txt', WIKI_BASE.'wiki.php?id=onlytextfoo#txt'),
            array('user', 'foo [\\]^`{|}~@+#%/#txt', WIKI_BASE.'wiki.php?id=user:foo#txt'),
            array('withquery', 'foo [\\]^`{|}~@+#%/#txt', WIKI_BASE.'wiki.php?id=anyns:foo&amp;do=edit#txt')
        );
