 ...
 */
abstract class EasyWikiTest extends PHPUnit\Framework\TestCase {
    ...
    /**
     * Reset the EasyWiki environment before each test run. Makes sure loaded config,
    ...
    public function setUp() : void {
        ...
        global $INPUT;
        $INPUT = new \easywiki\Input\Input();
    }
    ...
    public static function setupConfDir() {
        ...
        // clear any leftovers
        if(is_dir(WIKI_CONF)) {
            TestUtils::rdelete(WIKI_CONF);
        }
        mkdir(WIKI_CONF);

        // copy defaults
        foreach($defaults as $file) {
            copy(WIKI_INC . '/conf/' . $file, WIKI_CONF . $file);
        }
